...
export default function NavBar() {
    ...
    return (
        <nav className='flex justify-between rounded-xl p-4 shadow-[rgba(0,0,15,0.3)_0px_-5px_40px_-10px]'>
            ...
            <div className='flex items-center gap-2'><Link href="/updates" className='flex items-center justify-between flex-col'><Image src='UpdatesButton.svg' className='max-h-4' alt="" width={32} height={32} /><p className='text-[10px]'>Updates</p>{pathname == "/updates" && <Image src='SelectedTabBar.svg' alt="" width={32} height={32} />}</Link>{updates.filter(update => update.isEmergency).length > 0 && <Image src="Ellipse.svg" className="max-w-1.5 self-start" alt="" width={112} height={112} />}</div>
